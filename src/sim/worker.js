...
onmessage=(e)=>{try{const t=e.data.type,p=e.data.payload;
  ...
  else if(t==='pickSelect'){let best=null,bd2=1e9;for(const ent of entities){const dx=ent.x-p.x,dz=ent.z-p.z,d2=dx*dx+dz*dz;if(d2<bd2){bd2=d2;best=ent;}} if(best)postMessage({type:'selected',payload:{id:best.id,x:best.x,z:best.z,species:best.species,behavior:best.behavior,energy:best.energy,maxEnergy:maxEnergy(best.genes.size),hydration:best.hydration,age:best.age,diet:best.genes.diet}});}
