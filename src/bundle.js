...
(function(global){
  ...
  Engine3D.prototype.onResize=function(){const w=window.innerWidth,h=window.innerHeight;this.renderer.setSize(w,h);this.camera.aspect=w/h;this.camera.updateProjectionMatrix();};
    Engine3D.prototype.rebuildTerrain=function(terr){if(!terr||!terr.size)return;const THREE=this._THREE;const N=terr.size,H=terr.heights,B=terr.biomes;const step=terr.step||0.3,vs=terr.vs||8.0;this.terr.vs=vs;this.terr.waterLevel=terr.waterLevel;this.resMax=terr.resMax;this.baseColors=new Array(N*N);
  ...
  resScale&&resScale.addEventListener('input',function(){const newScale=parseFloat(this.value);sim&&sim.postMessage({type:'resourceScale', value:newScale});});
  resScaleReset&&resScaleReset.addEventListener('click',function(){if(resScale)resScale.value='1';sim&&sim.postMessage({type:'resourceScale', value:1.0});});
